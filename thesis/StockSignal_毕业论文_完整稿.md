...
## 6.6 大规模历史广度回测实证（4092 交易日）

前 6.1–6.5 节基于上线初期的小样本（n=4）验证了决策闭环的功能正确性与校准机制的设计正确性，但样本量不足以刻画策略在长周期上的统计行为。本节用重建的 **4094 个真实交易日**（2009–2026）全市场广度历史，对完整决策链路（`locate_cycle` + `derive_position`）做大规模历史回测，给出诚实的两类结论。
...
**表 6-3　各情绪周期下平均建议仓位（4092 交易日回测）**

| 情绪周期 | n | 平均建议仓位 avg_pct | 语义 |
|---|---|---|---|
| 冰点 | 7 | 5.0 | 极端风控封底 |
| 修复试探 | 2990 | 50.6 | 方向未明，观测为主 |
| 修复确认 | 421 | 59.7 | 右侧拐点，顺势加 |
| 主升高潮 | 293 | 55.2 | 趋势最一致 |
| 高潮分化 | 1 | 95.0 | 一致预期末端（n=1 噪声） |
| 退潮 | 380 | 35.0 | 亏钱效应扩散，防守 |

按四大战术分组聚合，进攻期平均 57.8% / 修复期 50.5% / 防守期 35.0%，**价差 22.8 个百分点**，单调且符合"进攻多扛、防守收缩"的预期。
...
**小结。** 本节把第 5 章的"可解释 + 可自纠偏"设计落到 4092 天真实数据上：方向不可预测被如实量化，而仓位刻度随周期分化的 22.8pt 价差则证明闭环的真实价值在于"择时仓位缩放"而非"方向博弈"。所有数字由 `backtest_decision_closure.py` 实算、可复现。
