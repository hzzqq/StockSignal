...
[![Tests](https://img.shields.io/badge/Tests-3393%20passed-brightgreen)](https://github.com/hzzqq/StockSignal)
...
| 🧪 **靠不靠谱** | **3393 个自动化测试**（含数据正确性断言、页面冒烟、后端安全回归），每个页面都有测试兜底 |
...
├── tests/ + backend/tests/  # 321 个测试文件 / 3393 用例
...
- **测试 3393 passed**：数据正确性断言（OHLC 自洽 / 日期单调 / 股息率反推区间）、68 页离线冒烟（整批不卡死）、后端安全回归 12/12
