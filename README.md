...
[![Tests](https://img.shields.io/badge/Tests-3358%20passed-brightgreen)](https://github.com/hzzqq/StockSignal)
...
> 68 个功能页面开箱即用——从行情看板到事件追踪，从策略回测到模拟交易。

⭐ **如果你觉得有用，点个 Star，或者直接提 Issue —— 每一条反馈都是这个项目活下去的氧气。** ⭐

📄 **[项目作品集展示页](https://hzzqq.github.io/StockSignal/)** —— 一页看完亮点 / 规模数据 / 实证结果 / 工程护栏，
全部由 [`scripts/gen_portfolio_page.py`](scripts/gen_portfolio_page.py) 从仓库真实数据实时生成；
源文件 [`docs/index.html`](docs/index.html) 为单文件自包含（图片已内联），可直接离线打开或转发。
...
| 📊 **功能全不全** | **68 个功能页面**：行情看板 / 个股分析 / 多股对比 / 事件追踪 / 策略回测 / 智能选股 / 资金流向 / 市场情绪 / 模拟交易 / 智能条件单…… |
| 🧪 **靠不靠谱** | **3358 个自动化测试**（含数据正确性断言、页面冒烟、后端安全回归），每个页面都有测试兜底 |
...
| 📊 **多股对比**<br/>5 只蓝筹归一化收益 | ![多股对比](screenshots/04-multi-stock-compare.png) | 📉 **回测引擎**<br/>成本模型 A/B 对照（摩擦成本 3.99pp） | ![回测曲线](screenshots/05-backtest-curve.png) |
...
│   pages/ (68页) + modules/    │ ──────▶ │  auth / stocks / admin / config│
│   双主题 · 数据正确性测试      │   JWT   └───────────────┬───────────────┘
└──────────────┬───────────────┘                         │ SQLAlchemy
               │                                         ▼
               │                    ┌────────────────────────────────────┐
...
├── tests/ + backend/tests/  # 320 个测试文件 / 3358 用例
...
- **测试 3358 passed**：数据正确性断言（OHLC 自洽 / 日期单调 / 股息率反推区间）、68 页离线冒烟（整批不卡死）、后端安全回归 12/12
