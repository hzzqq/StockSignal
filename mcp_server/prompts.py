...
- `get_realtime_quote(code)`：实时五档盘口（现价/涨跌/量能/买卖五档），回答「现在多少钱」。
- `get_market_sentiment(days=30)`：全市场情绪（8 项牧羊人指标 + 综合温度计 0-100），回答「市场恐慌/温度计」。
...
- 用户问「实时/现价/盘口」→ `get_realtime_quote(code)`，并用 A 股红涨绿跌着色描述涨跌。
- 用户问「市场情绪/恐慌/温度计」→ `get_market_sentiment(days)`，给温度数字 + 8 项指标解读。
...
SCENARIO_TEMPLATES: Dict[str, str] = {
    ...
    "实时盘口": (
        "请调 get_realtime_quote(code='{code}') 获取 {code} 的实时五档行情。\n"
        "用 A 股红涨绿跌着色描述现价、涨跌额、涨跌幅；列出今开/昨收/最高/最低；\n"
        "若买一卖一价差过大提示流动性风险；最后附一句「实时行情仅供参考」。\n"
        "若返回 error（非交易时间或数据源不可用），如实告知用户。"
    ),
    "市场情绪": (
        "请调 get_market_sentiment(days={days}) 获取全市场情绪温度计。\n"
        "解读：综合温度 0-100 的含义（>=75 亢奋、55-75 偏热、45-55 中性、25-45 偏冷、<25 冰点）；\n"
        "逐一看 8 项牧羊人指标（涨跌家数/涨停跌停/炸板率/连板高度/昨日涨停表现/红盘占比）\n"
        "是偏热还是偏冷，给出「当前应进攻还是防守」的一句话结论 + 风险提示。"
    ),
}
