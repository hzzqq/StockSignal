...
**研究智能体（H1，2026-09-17）**：`modules/research_agent.py` 复用同一网关做**多步工具调用
编排**——星辰AI 页开启「深度研究模式」后，问题经规划器（LLM 可用则 LLM 规划，否则确定性
关键词路由）挑选工具 → 逐步执行（12s/步超时 + 整轮时间预算 + 步数上限 8）→ 带引用综合。
每一步与数据时点（as_of）全程留痕，回答附 [S#] 引用可溯源；工具失败不编造，
全失败如实报 `unavailable`。网关工具表已改为从 MCP 注册表自动兜底（手抄 11 个 → 全量 16 个，
此后新增工具自动可用）。只读红线：智能体对 `conditional_orders` 仅放行 `action=list`。
