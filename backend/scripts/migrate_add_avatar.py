"""
backend/scripts/migrate_add_avatar.py
-------------------------------------
一次性迁移：为 users 表新增 avatar 列（若不存在）。

- 仅在 SQLite 后端执行（后端使用 sqlite:///...）。
- 用 PRAGMA table_info 探测列是否存在，缺失则 ALTER TABLE 加 TEXT 可空列。
- init_db.py 会在建表后自动调用本迁移；也可单独运行：
    python -m backend.scripts.migrate_add_avatar
"""
...
def main() -> None:
    ...
    with app.app_context():
        changed = migrate(app)
    print("[+] avatar 迁移：" + ("已加列" if changed else "列已存在，跳过"))
