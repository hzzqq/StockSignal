...
with c_mode:
    st.metric('当前模式', '🟢 实盘' if live else '⚪ 模拟', help='实盘模式：触发真实资金下单')
with c_cash:
...
with c_loss:
    ...
    st.metric('当日亏损停手线', label, help='当日资金净流出异常时自动停手（可在配置区手动解除）')
st.markdown('#### ⚙️ 账户配置')
with st.container(border=True):
    col_cfg1, col_cfg2 = st.columns([1.4, 1])
    with col_cfg1:
        broker_type = st.selectbox('券商通道', ['sim', 'qmt', 'easytrader'], index=['sim', 'qmt', 'easytrader'].index(bt), format_func=lambda x: {'sim': '模拟账本（默认，虚拟撮合）', 'qmt': 'QMT（迅投极速交易）', 'easytrader': '同花顺客户端（easytrader）'}[x], help='选择 QMT / 同花顺客户端 后需填写连接参数并开启实盘模式')
        live_mode = st.toggle('开启实盘模式（真实下单）', value=live, help='⚠️ 开启后将以真实资金下单！仅当已正确配置真实券商时启用。')
    with col_cfg2:
        max_order_amount = st.number_input('单笔金额上限 (元)', min_value=1000.0, step=1000.0, value=float(acc.get('max_order_amount') or 50000.0), help='单笔委托金额超过该值将被风控拒绝')
        daily_loss_limit = st.number_input('当日亏损停手线 (元)', min_value=1000.0, step=1000.0, value=float(acc.get('daily_loss_limit') or 20000.0), help='当日净流出触发该线后自动停手（保守保护）')
    ...
    if broker_type in ('qmt', 'easytrader'):
        ...
        if broker_type == 'qmt':
            with cc1:
                qmt_path = st.text_input('QMT 客户端路径', value=str(cfg.get('qmt_path') or ''), placeholder='如 C:/国金QMT/userdata_mini', help='QMT 客户端 userdata_mini 目录')
            with cc2:
                account_id = st.text_input('资金账号', value=str(cfg.get('account_id') or ''), placeholder='您的券商资金账号')
            broker_config = {'qmt_path': qmt_path, 'account_id': account_id}
        else:
            with cc1:
                use = st.text_input('客户端类型', value=str(cfg.get('use') or 'ths'), placeholder='ths / universal / joinquant', help='easytrader 的客户端标识')
                exe_path = st.text_input('客户端 exe 路径', value=str(cfg.get('exe_path') or ''), placeholder='如 C:/同花顺/xiadan.exe')
            with cc2:
                prepare_file = st.text_input('自动登录配置文件', value=str(cfg.get('prepare_file') or ''), placeholder='可选，自动登录 prepare 文件')
                old_pwd = cfg.get('password', '')
                pwd_placeholder = old_pwd if old_pwd and old_pwd != '******' else '******'
                password = st.text_input('交易密码', value=pwd_placeholder, type='password', help='留空或保持 ****** 表示沿用已保存密码')
            broker_config = {'use': use, 'exe_path': exe_path, 'prepare_file': prepare_file, 'password': password}
...
with st.container(border=True):
    ...
    with o2:
        side = st.selectbox('方向', ['buy', 'sell'], format_func=lambda x: '买入 ▲' if x == 'buy' else '卖出 ▼')
    with o3:
    ...
    if st.button('🚀 提交委托', type='primary', use_container_width=True, key='trade_submit'):
        if not order_code:
            ...
            try:
                ...
                pass
            payload = {'stock_code': order_code, 'stock_name': name, 'side': side, 'quantity': int(quantity), 'price': price if price > 0 else None}
...
def fragment_positions():
    ...
    for p in rows:
        ...
        with st.container(border=True):
            ...
            with cc4:
                st.markdown(f"<span style='color:{color};font-weight:700;'>{('+' if pnl >= 0 else '')}{pnl:,.0f} ({pnl_pct:+.2f}%)</span>", unsafe_allow_html=True)
...
def fragment_orders():
    ...
    for o in rows:
        ...
        status_cls = {'filled': 'sf-pill up', 'rejected': 'sf-pill down', 'failed': 'sf-pill down', 'submitted': 'sf-pill mid'}.get(status, 'sf-pill mid')
        with st.container(border=True):
            cm1, cm2, cm3, cm4 = st.columns([1.4, 1, 1, 1.4])
            with cm1:
                st.markdown(f'**{name}** `{code}`')
                st.caption(f"{('买入' if side == 'buy' else '卖出')} {int(o.get('quantity') or 0)} 股 @ {float(o.get('price') or 0):.3f}")
            with cm2:
                st.markdown(f"<span style='color:{st_color};font-weight:700;'>{('买入▲' if side == 'buy' else '卖出▼')}</span>", unsafe_allow_html=True)
            ...
            with cm4:
                st.caption(f"来源：{('条件单' if src == 'conditional' else '手动')}" + (f" · #{o.get('cond_order_id')}" if o.get('cond_order_id') else ''))
                msg = o.get('message')
                if msg:
                    st.caption(f'ℹ️ {msg}')
fragment_positions()
