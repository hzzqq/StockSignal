...
TRIGGER_LABELS = {
    "margin_stock": "💰 单股融资买入额 ≥ 阈值（万元）",
    "margin_market": "🌐 全市场融资买入额 ≥ 阈值（亿元）",
    "ma5_break_up": "📈 沿 5 日均线上涨突破（上穿 MA5）",
    "ma5_break_down": "📉 沿 5 日均线下跌破位（跌破 MA5）",
}
STATUS_LABELS = {
    "pending": "⏳ 待触发",
    "triggered": "⚡ 已触发待成交",
...
with st.expander("💡 触发类型说明", expanded=False, key="cond_help"):
    st.markdown(
        "- **单股融资买入额**：抓该股最近披露日的融资买入额，达到设定「万元」阈值即触发（融资余额高 = 资金做多意愿强）。\n"
...
with st.expander("➕ 新建条件单", expanded=True, key="new_cond_exp"):
    ...
    with c2:
        trigger_type = st.selectbox(
            "触发条件", list(TRIGGER_LABELS.keys()),
            format_func=lambda x: TRIGGER_LABELS[x],
        )
        action = st.selectbox("触发动作", ["buy", "sell"],
    ...
    with p1:
        if trigger_type in ("margin_stock", "margin_market"):
            ...
            threshold = st.number_input(
                f"融资买入额阈值（{unit}）", min_value=0.0, step=0.1,
                value=5000.0 if trigger_type == "margin_stock" else 1000.0,
                help="达到该阈值即触发",
            )
            params["threshold"] = float(threshold)
        else:
            confirm_pct = st.number_input(
                "突破确认幅度 (%)", min_value=0.0, max_value=10.0, step=0.1, value=0.0,
                help="0 = 只要穿越即触发；>0 需突破该幅度才确认",
            )
            params["confirm_pct"] = float(confirm_pct)
    with p2:
        quantity = st.number_input("下单数量 (股)", min_value=100, step=100, value=100,
                                   help="须为 100 的整数倍")
    with p3:
        default_expire = (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d")
        expire_date = st.date_input(
            "到期日（可选）", value=datetime.strptime(default_expire, "%Y-%m-%d"),
            help="超过该日期未触发则自动失效",
        )
    ...
    if st.button("🚀 创建条件单", type="primary", use_container_width=True, key="cond_create"):
        if not cond_code:
            ...
            try:
                ...
                pass
            payload = {
                "stock_code": cond_code, "stock_name": name,
                "trigger_type": trigger_type, "action": action,
                "trigger_params": params, "quantity": int(quantity),
...
with col_scan:
    if st.button("🔍 立即扫描一轮", type="primary", use_container_width=True, key="cond_scan"):
        ...
        if sc == 200 and isinstance(body, dict):
            ...
            st.success(
                f"扫描完成：检查 {stats.get('checked',0)} · 触发 {stats.get('triggered',0)} · "
                f"成交 {stats.get('filled',0)} · 失败 {stats.get('failed',0)} · 过期 {stats.get('expired',0)}"
            )
        else:
            st.error("扫描失败")
with col_info:
    st.caption("调度器会在交易时段（周一至周五 9:15-11:35 / 12:55-15:05）自动扫描；"
               "此按钮可手动触发全量扫描。")
...
def fragment_cond_list():
    ...
    # 过滤
    _only_active = st.checkbox("只看生效中", key="cond_only_active",
                               help="仅显示 pending 且 active=True 的条件单")
    if _only_active:
    ...
    for idx, c in enumerate(rows):
        ...
        status_cls = {
            "pending": "sf-pill mid",
            "triggered": "sf-pill up",
            "filled": "sf-pill up",
            "failed": "sf-pill down",
            "cancelled": "sf-pill down",
            "expired": "sf-pill mid",
        ...
        with st.container(border=True):
            ...
            with cm2:
                st.markdown(f"{desc}")
                st.caption(f"触发后：{'买入▲' if action=='buy' else '卖出▼'} "
                           f"{int(c.get('quantity') or 0)} 股"
                           + (f" · 到期 {c.get('expire_date')}" if c.get('expire_date') else ""))
            with cm3:
