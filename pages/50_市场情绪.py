...
from modules.ladder_view import render_ladder_block
...
st.page_link("pages/15_市场驱动力.py", label="📊 看《市场驱动力》五维归一化相关性分析（互补视角）", icon="🔗")
...
def fragment_shepherd_review():
    ...
    # ── 连板梯队全景（抽成共享组件 modules/ladder_view.render_ladder_block）──
    render_ladder_block(dark)
...
def fragment_shepherd_forecast():
    ...
    # I8：复盘归档（保存快照 + 历史回测）的唯一入口收敛到 54_今日决策面板，
