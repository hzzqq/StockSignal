...
def _fetch_watchlist_full():
    ...
    for it in body.get('data', []) or []:
        ...
        if not code:
            continue
        items.append({'id': it.get('id'), 'code': str(code), 'name': it.get('stock_name') or it.get('name') or str(code), 'note': it.get('note') or ''})
    return (items, None)
...
def fragment_sector():
    ...
    if not show_cols:
        ...
        return
    st.dataframe(top[show_cols], width="stretch", hide_index=True, column_config={'净额': st.column_config.NumberColumn('净额(亿)', format='%.2f'), '涨跌幅': st.column_config.NumberColumn('涨跌幅%', format='%.2f'), '流入资金': st.column_config.NumberColumn('流入(亿)', format='%.2f'), '流出资金': st.column_config.NumberColumn('流出(亿)', format='%.2f')}, height=400)
...
def fragment_watchlist():
    ...
    wl_only_alert = falert.checkbox('仅看预警', value=False, key='wl_only_alert', help='只显示触发涨跌异动阈值（预警页设置）的标的。')
    if st.button('🔄 清空筛选', key='wl_reset', help='清空名称/代码筛选、涨跌区间与「仅看预警」，恢复全部标的'):
    ...
    if not items:
        ...
        if st.button('👀 查看示例', key='wl_sample'):
            ...
            st.dataframe(_sample, width="stretch", hide_index=True, height=400)
        return
    ...
    for code in codes:
        ...
        rows.append({'代码': code, '名称': name, '现价': round(cur, 2) if cur is not None else None, '涨跌%': round(chg, 2) if chg is not None else None, '涨跌额': round(change_amt, 2) if change_amt is not None else None})
    df = pd.DataFrame(rows)
    df = df.sort_values('涨跌%', ascending=False, na_position='last').reset_index(drop=True)
    if wl_q:
        q = wl_q.strip().lower()
        df = df[df.apply(lambda r: q in str(r['代码']).lower() or q in str(r['名称']).lower(), axis=1)]
    lo, hi = wl_rng
    ...
    try:
        ...
        st.dataframe(styled, width="stretch", hide_index=True, column_config={'现价': st.column_config.NumberColumn(format='%.2f'), '涨跌%': st.column_config.NumberColumn(format='%.2f%%'), '涨跌额': st.column_config.NumberColumn(format='%.2f')}, height=400)
    except Exception:
        st.dataframe(df, width="stretch", hide_index=True, column_config={'现价': st.column_config.NumberColumn(format='%.2f'), '涨跌%': st.column_config.NumberColumn(format='%.2f%%'), '涨跌额': st.column_config.NumberColumn(format='%.2f')}, height=400)
...
def fragment_individual_ff():
    ...
    iff_dir = d1.selectbox('资金方向', ['全部', '仅净流入', '仅净流出'], index=['全部', '仅净流入', '仅净流出'].index(st.session_state.get('pref_iff_dir', '全部')), key='iff_dir')
    ...
    for code in codes:
        ...
        rows.append({'代码': code, '名称': name, '主力净流入(亿)': round(main_net / 100000000.0, 3) if isinstance(main_net, (int, float)) else None, '来源': '实时' if source == 'akshare' else '估算' if source == 'estimate' else '无数据'})
    df = pd.DataFrame(rows)
    ...
    if iff_q.strip():
        q = iff_q.strip().lower()
        valid = valid[valid.apply(lambda r: q in str(r['代码']).lower() or q in str(r['名称']).lower(), axis=1)]
    if iff_dir == '仅净流入':
    ...
    with col_l:
        ...
        try:
            ...
            st.dataframe(styled, width="stretch", hide_index=True, column_config={'主力净流入(亿)': st.column_config.NumberColumn(format='%.3f')}, height=400)
        except Exception:
            st.dataframe(top_in, width="stretch", hide_index=True, column_config={'主力净流入(亿)': st.column_config.NumberColumn(format='%.3f')}, height=400)
    with col_r:
        ...
        try:
            ...
            st.dataframe(top_out, width="stretch", hide_index=True, column_config={'主力净流入(亿)': st.column_config.NumberColumn(format='%.3f')}, height=400)
    ...
    if any((r.get('source') == 'estimate' for r in ff_map.values())):
        st.caption('⚠️ 标注「估算」的数据为量价模型（Chaikin 风格）估算的主力净流入，仅反映近期量价博弈方向，非交易所逐笔主力数据。')
...
def fragment_alerts():
    ...
    threshold = st.slider('涨跌异动阈值 (%)', min_value=0.5, max_value=10.0, value=3.0, step=0.5, key='smart_alert_threshold', help='自选股当日涨跌%绝对值超过该阈值即触发「异动预警」。')
    ...
    for code in codes:
        ...
        if chg is not None and abs(chg) >= threshold:
            ...
            alerts.append({'类型': '异动预警', '代码': code, '名称': name, '说明': f'涨跌 {chg:+.2f}% ≥ 阈值 {threshold:.1f}%（{label}）', 'tier': tier, 'color': color})
        r = ff_map.get(code, {})
        main_net = r.get('main_net')
...
def fragment_watch_manage():
    ...
    add_q = a1.text_input('➕ 添加关注（6 位代码）', value='', key='wl_add_q', placeholder='如 600519', help='输入 6 位数字代码（如 600519）后点击「添加」；也可在行情看板或形态选股中一键关注。')
    ...
    if a2.button('添加', key='wl_add_btn', width="stretch"):
        raw = (add_q or '').strip()
        if not raw:
            xc_warn_box('请输入代码')
        elif raw.isdigit():
    ...
    for it in items:
        ...
        c3.markdown(f"<span style='color:{cc};font-weight:600;'>{chg:+.2f}%</span>" if chg is not None else '—', unsafe_allow_html=True)
        ...
        if st.session_state.get(_ck):
            ...
            if c5.button('取消', key=f'wm_rm_cancel_{code}'):
                st.session_state.pop(_ck, None)
        elif c5.button('移除', key=f'wm_rm_{code}'):
