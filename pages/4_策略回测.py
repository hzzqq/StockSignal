"""
页面4：策略回测
支持四种策略：趋势动量多因子（推荐）、双趋势共振 GMMA+一目（强势股）、均线交叉、事件驱动
...
移植自「双趋势共振量化选股系统」：**两套独立趋势系统同时看多才入场，任一走坏即离场**。

| 系统 | 作用 | 判定 |
|-----|------|------|
| **GMMA 顾比均线** | 主趋势结构 | 短期组(EMA3-15)均值 > 长期组(EMA30-60)均值，且 EMA15 > EMA30 |
| **一目均衡表** | 第二重确认 + 防线 | 价格站上云层（短区间云未形成时退化为转换线>基准线） |
| **ADX 趋势过滤** | 拒绝震荡市 | ADX ≥ 20 才允许入场，过滤假突破 |

**入场**：GMMA 多头 + 一目看多 + ADX≥20 三者共振。
**离场**：GMMA 转空（EMA15 < EMA30）或收盘跌破基准线 kijun。

> **与多因子策略的互补**：本策略**完全不看 RSI**，纯趋势跟踪——长电科技这类
> 「长期 RSI>85 的强势上涨股」只要均线多头结构不破坏就一路持有，从机制上根治
> 强势股被超买指标排除的问题。**建议**：止盈放大到 10% 以上或改用移动止损（5-8%），
> 让趋势利润奔跑；小止盈会过早截断本策略的优势。
""")
