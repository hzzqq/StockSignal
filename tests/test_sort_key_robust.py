...
背景（真实脆弱点）：
- pages/B_形态选股.py: results.sort(key=lambda r: r["技术评分"]) —— 形态扫描结果任一条
  缺 "技术评分" 字段（异常/部分输出）即 KeyError，整页崩溃。
- pages/I_体检扫描.py: _sort_key 用 r["priority"] / r["code"] 直接下标 —— 同样风险。
...
class TestSortKeyUsesGetNotSubscript:
    def test_b_page_uses_get_for_score(self):
        src = (ROOT / "pages" / "B_形态选股.py").read_text(encoding="utf-8")
        assert 'r.get("技术评分", 0)' in src
        assert 'r["技术评分"]' not in src

    def test_i_page_sort_key_uses_get(self):
        src = (ROOT / "pages" / "I_体检扫描.py").read_text(encoding="utf-8")
