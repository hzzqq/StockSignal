...
------------------------------------------------------------
``scripts/analyze_sentiment_predictive_power.py`` 把 2007–2026 的 4785 天逐日喂给
``forecast_next_day``，得到三个必须写进代码的结论（2026-09-11 P3 修正后）：

  1. **方向不可预测**：引擎的偏多/偏空输出在历史样本上近乎噪音（偏多仅 6 天、偏空 240 天，
     5 日尺度偏空 z=-10.27 显著为负）；0-100「次日情绪评分」与次日收益的 IC≈0（≈-0.03），
     十分位单调性 IC≈0.02。普通交易日的次日方向**不可预测**。
  2. **生产层只用尺度无关的占比类特征**：历史基座是逐年长大的**采样**（2009 年 26 只 →
     2025 年 1669 只），绝对家数阈值跨年代不可比；且 limit_down 已于 P1/P2 用 v1 缓存
     **全历史还原为真实值**（实证报告里「跌停>100家」现可触发 130 天），但生产层为跨年代
     一致仍只用占比类。
  3. **唯一经样本外 + Bonferroni 校正活下来的规律**：极端恐慌 → 次日反弹
     （walk-forward：跌停占比前10% 触发 296 天 62.5% vs 49.8% z=+4.36；
      样本外 top10% 次日上涨 64.0% z=+3.39。均用全历史可比的「次日红盘日」标签）。
     「触及跌停占比」因 touch_down 全历史缺失（仅近窗 14 天有值）→ 触发 0 天，**不发布**。
