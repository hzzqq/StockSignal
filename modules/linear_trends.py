...
def get_market_cumulative_series(days=60):
    ...
    def _fn():
        ...
        return out
    # 强边界：东方财富 urllib 路径可能无限挂起，12s 硬超时后返回空 DF，由 UI 兜底
    return _cached(600, f"market_cumulative_{days}", lambda: (lambda _r: _r if _r is not None else pd.DataFrame())(_run_with_timeout(_fn, 12)))
...
def get_industry_index_series(top_n=8, days=120):
    ...
    def _fn():
        ...
        return base
    # 强边界：东方财富 urllib 路径可能无限挂起，12s 硬超时后返回空 DF，由 UI 兜底
    return _cached(1800, f"industry_index_series_{top_n}_{days}", lambda: (lambda _r: _r if _r is not None else pd.DataFrame())(_run_with_timeout(_fn, 12)))
...
def get_etf_series(days=180):
    ...
    def _fn():
        ...
        return base
    # 强边界：东方财富 urllib 路径可能无限挂起，12s 硬超时后返回空 DF，由 UI 兜底
    return _cached(1800, f"etf_series_{days}", lambda: (lambda _r: _r if _r is not None else pd.DataFrame())(_run_with_timeout(_fn, 12)))
