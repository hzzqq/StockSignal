"""多股票横向对比 —— 渲染层（纯函数：dict -> HTML / Plotly）

本模块为叶子模块：仅依赖标准库、numpy、plotly（局部懒加载）与
modules.ui_theme（_is_dark 内局部懒加载），不反向依赖 modules.compare，
避免循环导入。样式令牌与共享小工具与数据层共用，统一在此定义。
"""
