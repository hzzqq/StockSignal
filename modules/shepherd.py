...
if __name__ == "__main__":
    ...
    if not df.empty:
        ...
        # 委托 modules.atomic_io（tmp 名唯一化 + replace 退避重试），见其模块文档：
        # 固定 tmp 名在多写者并发下会 PermissionError(WinError 32)，压测实测 160 次写入失败 7 次。
        _hist_dir = os.path.dirname(_HISTORY_FILE)
        atomic_to_csv(df, _HISTORY_FILE, encoding="utf-8-sig")
        _json_path = os.path.join(_hist_dir, "shepherd_history.json")
        _js = df.copy()
        _js["date"] = _js["date"].astype(str)
        atomic_json_dump(_js.to_dict(orient="records"), _json_path)
