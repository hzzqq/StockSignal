...
FORECAST_INDICATORS = [
    ...
    dict(
        key="ladder_promo", name="连板晋级率", unit="%", dir=1, weight=8, derived=True,
        why="首板→二板的晋级率 = 昨日首板今天有几成封住二板，是「接力意愿」最纯粹的度量，"
             "比连板家数更细（家数只数厚度，晋级率数质量）。"
             "≥25% 说明资金愿意持续顶一字/换手晋级，梯队能自我维持（正向延续）；"
             "<15% 说明首板次日即大量淘汰，梯队只剩独苗、赚钱效应难扩散（接力弱）。"
             "该值来自每日连板梯队落盘的跨日递推，需积累≥2日历史才有意义。",
        bands=[
            (0, 15, "接力弱", "首板次日大批淘汰，梯队难维持", "#00d486"),
            (15, 25, "接力一般", "有一定晋级，但不够稳", "#3b82f6"),
            (25, 40, "接力健康", "★ 资金持续晋级，梯队能扩散", "#f59e0b"),
            (40, 99999, "接力火爆", "高潮特征，注意盛极而衰", "#ee2a2a"),
        ],
    ),
]
...
LINKAGE_RULES = [
    ...
    dict(
        id="relay_promo_strong",
        name="晋级率健康（接力延续）",
        conds=[("ladder_promo", ">=", 25)],
        logic="连板晋级率≥25%",
        effect="首板→二板成功率高，梯队自我维持，赚钱效应可扩散，次日接力环境健康",
        color="#ee2a2a",
        tags=["看多次日", "接力"],
    ),
    dict(
        id="relay_promo_weak",
        name="晋级率低迷（接力弱）",
        conds=[("ladder_promo", "<", 15)],
        logic="连板晋级率<15%",
        effect="首板次日大量淘汰，梯队只剩独苗，赚钱效应难扩散，次日偏谨慎",
        color="#00d486",
        tags=["看空次日", "接力弱"],
    ),
]
