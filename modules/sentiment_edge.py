...
把 2009–2026 的 4094 个交易日逐日喂给 ``forecast_next_day`` 实测后，结论是**残酷但清楚**的：
  ...
  ├ 手工规律大多无法验证
  │   · 「跌停>100家」触发率 0.32%（n=5）、「涨停≥80家」0.52%（n=8）——**规则死在不匹配**
  │   · 根因：历史基座是**逐年长大的采样**（2009 年 26 只 → 2025 年 1669 只），
  │     家数被系统性低估约 3 倍，针对全市场写的绝对阈值几乎永不触发
  └ 唯一的真信号：**极端恐慌 → 次日反弹**
      · walk-forward（第 i 天阈值只用第 i 天之前的数据）验证：
          跌停占比前10%    触发 78 天 → 次日上涨 66.7%（基准 46.5%）z=+3.57
          触及跌停占比前10% 触发 65 天 → 次日上涨 63.1%（基准 46.5%）z=+2.68
      · 20 次样本外尾部检验 + Bonferroni 校正（α=0.0025）后只有这两个活下来
...
明确的能力边界（写进产品文案，不藏）
------------------------------------
    ✅ 可信：极端恐慌 → 次日反弹概率显著高于基准（约 +17~20pp）
    ...
    if not hits:
        ...
        return dict(available=True, triggered=False, evaluated=True, hits=[], prob=None,
                    ...
                    statement=("当日情绪未进入历史极值区间：**次日方向无统计边际，本模块不表态**"
                               "（现有情绪评分与次日收益 IC≈0，经 1547 天检验无法超越基准率）。"),
                    ratios=r)
