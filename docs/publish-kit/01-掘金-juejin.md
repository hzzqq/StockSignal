...
我开源了一个 A 股行情数据研究工具：4 大免费数据源自动降级 + 68 个页面 + 3358 个测试
...
软件工程实训课程设计。从想研究「事件与行情的关系」开始，慢慢堆出了 68 个功能页面——行情看板、事件追踪、策略回测、资金流向、市场情绪、模拟组合、条件提醒、AI 研究助手……把自己能想到的 A 股数据场景都做了进去。

**核心能力**

- 4 级数据源降级链（AKShare → BaoStock → 新浪 → 东财 → 本地缓存）：单点故障不崩，断网也能看历史
- 68 个功能页面，暗夜/白天双主题，A 股红涨绿跌
- 3358 个自动化测试（320 个测试文件）：数据正确性断言（OHLC 自洽、日期单调）+ 全页面离线冒烟 + 后端安全回归
